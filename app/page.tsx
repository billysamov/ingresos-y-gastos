...
export default function Home() {
  ...
  function addDetailedExpense(e:React.FormEvent<HTMLFormElement>) {
    ...
    if(expenseModal.kind==="sub"&&expenseModal.groupId) {
      ...
      if(rowCategories.some(entry=>entry==="Ahorro")) { setNotice("Ahorro se registra desde Movimientos para elegir su destino"); return; }
  ...
  function saveExpenseEdit(e:React.FormEvent<HTMLFormElement>) {
    ...
    else { const category=String(fd.get("category")||"Otros"); if(category==="Ahorro") { setNotice("Ahorro se registra desde Movimientos para elegir su destino"); return; } setExpenseGroups(groups=>groups.map(group=>group.id===expenseEdit.groupId?{...group,items:group.items.map(item=>item.id===expenseEdit.itemId?{...item,name,amount,category}:item)}:group)); }
